...
class EventSelectionMergerConfig(ConfigBlock):
    ...
    def makeAlgs(self, config):
        alg = config.createAlgorithm('top::SaveFilterAlg', 'EventSelectionMerger')
...
class EventSelectionConfig(ConfigBlock):
    ...
    def raise_misconfig(self, text, keyword):
        raise ValueError (f"Misconfiguration! Check {keyword} in: {text}")
    ...
    def check_float(self, test, requirePositive=True):
        try:
            ...
            if not requirePositive or value >= 0:
                ...
                raise ValueError (f"Misconfiguration! Float {test} is not positive!")
        except ValueError:
            raise ValueError (f"Misconfiguration! {test} should be a float, not {type(test)}!")
        ...
        try:
            ...
            if value == float(test):
                if not requirePositive or value >= 0:
                    ...
                    raise ValueError (f"Misconfiguration! Int {test} us not positive!")
            else:
                raise ValueError (f"Misconfiguration! {test} should be an int, not a float!")
        except ValueError:
            raise ValueError (f"Misconfiguration! {test} should be an int, not {type(test)}")

    def check_string(self, test):
        if not isinstance(test, str):
            raise ValueError (f"Misconfiguration! {test} should be a string, not a number!")
        else:
            return test
    ...
    def check_sign(self, test):
        ...
        try:
            ...
            raise KeyError (f"Misconfiguration! {test} should be one of {list(mapping.keys())}")

    def check_btagging(self, test):
        test = test.split(":")
        if len(test) != 2:
            raise ValueError (f"Misconfiguration! {test} should be provided as 'btagger:btagWP'")
        else:
            return test
    ...
    def add_MET_selector(self, text, config):
        ...
        alg = config.createAlgorithm('top::METSelectorAlg', thisalg)
    ...
    def add_MWT_selector(self, text, config):
        ...
        alg = config.createAlgorithm('top::MWTSelectorAlg', thisalg)
        alg.met = config.readName(self.met)
    ...
    def add_METMWT_selector(self, text, config):
        ...
        alg = config.createAlgorithm('top::METMWTSelectorAlg', thisalg)
        alg.met = config.readName(self.met)
        alg.electrons, alg.electronSelection = config.readNameAndSelection(self.electrons)
        alg.muons, alg.muonSelection = config.readNameAndSelection(self.muons)
        alg.sign = self.check_sign(items[1])
    ...
    def add_MLL_selector(self, text, config):
        ...
        alg = config.createAlgorithm('top::MLLSelectorAlg', thisalg)
        alg.electrons, alg.electronSelection = config.readNameAndSelection(self.electrons)
        alg.muons, alg.muonSelection = config.readNameAndSelection(self.muons)
        alg.sign = self.check_sign(items[1])
    ...
    def add_MLLWINDOW_selector(self, text, config):
        ...
        alg = config.createAlgorithm('top::MLLWindowSelectorAlg', thisalg)
        alg.electrons, alg.electronSelection = config.readNameAndSelection(self.electrons)
        alg.muons, alg.muonSelection = config.readNameAndSelection(self.muons)
        alg.highMLL = self.check_float(items[1])
        alg.lowMLL = self.check_float(items[2])
        alg.eventPreselection = f'{self.currentDecoration}'
        self.setDecorationName(alg, config, f'{thisalg}_%SYS%')
        return
    ...
    def add_OS_selector(self, text, config):
        ...
        alg = config.createAlgorithm('top::ChargeSelectorAlg', thisalg)
        alg.electrons, alg.electronSelection = config.readNameAndSelection(self.electrons)
        alg.muons, alg.muonSelection = config.readNameAndSelection(self.muons)
        alg.OS = True
        alg.eventPreselection = f'{self.currentDecoration}'
    ...
    def add_SAVE(self, text, config):
        ...
        alg = config.createAlgorithm('top::SaveFilterAlg', thisalg)
