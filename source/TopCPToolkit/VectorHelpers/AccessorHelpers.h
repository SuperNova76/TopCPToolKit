 ...
 *
 * PartonAccessor provides methods to initialize, retrieve, and apply accessors for various
 * properties like mass, pt, eta, phi, and PDG ID. It manages both float and integer accessors.
 */
struct PartonAccessor {
  ...
  void initializeFloatAccessor(const std::string& name) {
    floatAccessors[name] = std::make_unique<SG::AuxElement::Accessor<float>>(name);
  }
  ...
  void getPtEtaPhiMVector(const std::string& prefix, ROOT::Math::PtEtaPhiMVector& p, T* object){
    p.SetCoordinates(getFloat(prefix + "_pt", object),
		     getFloat(prefix + "_eta", object),
		     getFloat(prefix + "_phi", object),
		     getFloat(prefix + "_m", object));
  }
};
