...
RunNuFlowsAlg::RunNuFlowsAlg(const std::string &name, ISvcLocator *pSvcLocator)
    : EL::AnaAlgorithm(name, pSvcLocator), m_onnxFilePath("SetMe"), m_btagger("SetMe"), m_write_inputs(false) {
    ...
    declareProperty("write_inputs", m_write_inputs, "Save the inputs to the network in the EventInfo object");
}
