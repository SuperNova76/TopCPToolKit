...
namespace top {
  ...
  MWTSelectorAlg::MWTSelectorAlg(const std::string &name, ISvcLocator *pSvcLocator)
  : EL::AnaAlgorithm(name, pSvcLocator)
  , m_mwtref(0.)
  , m_sign("SetMe")
  {
    declareProperty ("refMWT", m_mwtref, "MWT cut (in MeV)");
    declareProperty ("sign", m_sign, "comparison sign to use");
  }

  StatusCode MWTSelectorAlg::initialize() {
  ...
  StatusCode MWTSelectorAlg::execute() {
    for (const auto &sys : m_systematicsList.systematicsVector()) {
      ...
      // calculate decision
      bool decision = SignEnum::checkValue(m_mwtref, m_signEnum, mwt);
      m_decoration.setBool(*evtInfo, decision, sys);
    }
    return StatusCode::SUCCESS;
  }

  StatusCode MWTSelectorAlg::finalize() {
    return StatusCode::SUCCESS;
  }
} // namespace top
