...
TopSpaNetTtbarLjetsNu::TopSpaNetTtbarLjetsNu(const std::string& name, const std::string& model_even, const std::string& model_odd) : TopSpaNetTopology(name, model_even, model_odd),
                                                                                                                                     m_lep_b(-1),
                                                                                                                                     m_had_b(-1),
                                                                                                                                     m_down(-1),
                                                                                                                                     m_up(-1),
...
int input_jet_size = 1;
for (unsigned long int i = 0; i < input_jet_tensor_dims.size(); ++i) {
    input_jet_size = input_jet_size * input_jet_tensor_dims[i];
}
this->addInputs(
    **jet_values, input_jet_size, input_jet_tensor_dims.data(), input_jet_tensor_dims.size());
int jet_mask_size = 1;
