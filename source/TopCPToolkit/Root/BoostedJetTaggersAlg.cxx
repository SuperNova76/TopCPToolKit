...
namespace top {
  ...
  StatusCode BoostedJetTaggersAlg::initialize() {
    ...
    m_ljetsTopTagPassKinRangeHandle = std::make_unique<CP::SysWriteDecorHandle<int>> (
      this,
      m_taggerDecoration  + "_validKinRange",
    ...
    m_ljetsTopTagEfficiencyHandle = std::make_unique<CP::SysWriteDecorHandle<float>> (
      this,
    ...
    ANA_CHECK(m_ljetsTopTagPassKinRangeHandle->initialize(m_systematicsList, m_ljetsHandle));
  ...
  StatusCode BoostedJetTaggersAlg::execute() {
    ...
    const SG::ConstAccessor<bool> acc_ValidKinRange(m_taggerDecoration + "_ValidKinRange");
    ...
    for (const auto &sys : m_systematicsList.systematicsVector()) {
      ...
      for(size_t ijet = 0; ijet < shallowJets->size(); ijet++) {
        ...
        const int validKinRange = acc_ValidKinRange.isAvailable(*scJet) ? acc_ValidKinRange(*scJet) : 0;
